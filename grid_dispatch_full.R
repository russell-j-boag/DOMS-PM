...
# Load
load("samples/sDOMSPM_full.RData")
...
# Save samples
save(samples, samples1, file = "samples/sDOMSPM_full.RData")
...
#                       report = 10)
# # Save samples
# save(samples, samples1, samples2, file = "samples/sDOMSPM_full.RData")
...
# Chains
pdf("plots/chains_full.pdf", height = 6, width = 8)
par(mfrow = c(3, 3))
...
# Chains by subject
pdf("plots/chains_by_sub_full.pdf", height = 6, width = 8)
...
save(post_preds, file = "deriv/post_preds_full.RData")

# Load posterior predictives
load("deriv/post_preds_full.RData")

# Plot fits to CDFs
pdf("plots/cdfs_full.pdf", height = 6, width = 8)
plot.pp.dmc(post_preds, "cdf", layout = c(2, 2), model.legend = FALSE)
dev.off()

# Subject-level fits
pdf("plots/cdfs_by_sub_full.pdf", height = 6, width = 8)
...
head(parms)
save(parms, file = "deriv/map_parms_full.RData")
...
# # Correlations
# pdf("plots/pairs_full.pdf", height = 6, width = 8)
